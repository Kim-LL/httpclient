...
public class MainClientExec implements ClientExecChain {
    ...
    public CloseableHttpResponse execute(
        ...
        try {
            ...
            // 此时这个 managedConn 就是一个 CPoolProxy 里面包含了链接, connRequest 是一个 PoolingHttpClientConnectionManager
            // 创建一个管理链接对象，这个就是用来做链接的，下面的步骤是真正的链接时间
            long start = System.currentTimeMillis();
            managedConn = connRequest.get(Math.max(timeout, 0), TimeUnit.MILLISECONDS);
            long end = System.currentTimeMillis();
            if(managedConn instanceof CPoolProxy && managedConn.isOpen()){
                // 如果这个能直接获取到链接，说明之前链接过，那我就直接算链接时间
                ((CPoolProxy)managedConn).setConnectElapseMillisecond(end - start);
            }
